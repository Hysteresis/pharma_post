...
def find_next_date(request):
    if request.method == 'GET':
        ...
        if current_date:
            ...
            if next_date:
                geographies_63 = D_Geographie.objects.filter(pk_geographie__startswith='63').values_list('pk_geographie', flat=True)
                doses_for_next_date = F_Dose.objects.filter(fk_date=next_date, fk_geographie__in=geographies_63)
